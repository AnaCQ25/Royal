...
<div class="col-md-12 mt-lg-4 mt-4">
   ...
   <div class="main">
    <form action="editar_perfil_estudiante.php" id="frm_foto" role="form" method="post" enctype="multipart/form-data">
    <div class="row">
        <div class="col col-6">
              <div class="row">
          ...
          <br>
            <p size="10px"><code style: font-size: 10px;>Nota:</code> Debe darle al botón de subir para poder guardar la imagen y establecerla <br> de foto de perfil.</p>
          </div>
          </div>
        </div>
              ...
              <div class="row">
            <!-- Usuario -->
           <label for="Nombre"><b> Matricula </b></label>
           <input class="form-control" type="text" id="matricula" name="matricula" value="<?= $user['matricula']; ?>" maxlength="50" required disabled>
         </div>
             <p></p>
      ...
      <div class="col-md">
          ...
          <div class="form-check">
           ...
           <input class="form-control" type="text" id="email" name="email" value="<?= $user['email']; ?>" placeholder="Ingresa tu email" maxlength="50"  >
            </div>
          </div>
           <p></p>
          ...
          <center>
          <!-- BOTON Enviar -->
          <input type="hidden" name="id" value="<?php $_SESSION['user_id2']; ?>">
...
$('#bar').click(function(){
 ...
 <?php
  ...
  if (isset($_POST['subir'])){
        ...
        $sql2 = "UPDATE `estudiante` SET `foto` = :foto WHERE `id` = :id";
        $stmt2 = $conn->prepare($sql2); 
        $stmt2->bindParam(':foto', $_FILES['file']['name']); 
        $stmt2->bindParam(':id', $_SESSION['user_id2']);  
       ...
       location.replace('http://localhost/Royal/editar_perfil_estudiante.php');</script>";
...
$sql1 = "UPDATE `estudiante` SET `nombre` = :nombre, `apellido` = :apellido, `genero` = :genero, `email` = :email, `telefono` = :telefono, `celular` = :celular, `direccion` = :direccion WHERE `id` = :id";
...
$stmt1->bindParam(':id', $_SESSION['user_id2']); 
...
if ($stmt1->execute()){
 ...
 location.replace('http://localhost/Royal/editar_perfil_estudiante.php');</script>";
  ...
  'Inténtelo de nuevo.',
  'error'
); windows.location='editar_perfil_estudiante.php';</script>";
}
